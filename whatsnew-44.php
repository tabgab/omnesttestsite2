...
<ul>
  <li>
    ...
    in the module's or channel's NED description; undeclared signals
    will result in a runtime error. Signal declarations are of the form
  ...
  <li>Feature: Just-in-time debugging support.
    <ul>
      <li>new configuration options: <em>debugger-attach-on-startup</em>, <em>debugger-attach-on-error</em>,
        <em>debugger-attach-command</em>, <em>debugger-attach-wait-time</em>.</li>
      <li>the simulation kernel can invoke the debugger on an error (or on startup)
        using the command-line specified by the <em>debugger-attach-command</em> option.</li>
    </ul>
  </li>
</ul>
...
<ul>
  ...
  <li>Fix: the "Filter Window Contents" dialog could lose state</li>
  <li>Source cleanup (consistent naming style for Tcl procedures)</li>
</ul>

<p>IDE:</p>

<ul>
  <li>Use Eclipse 4.3 as base platform</li>
  <li>
...
<ul>
  ...
  <li>Added proper support for cross-compilation using "./configure --host=i686-w64-mingw32"</li>
  <li>
  ...
  <li>IDE: fixed the launcher not to throw error if the toolchain is clang.</li>
